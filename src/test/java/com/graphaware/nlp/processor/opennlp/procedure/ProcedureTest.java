 ...
 */
package com.graphaware.nlp.processor.opennlp.procedure;
...
//        clean();
    ...
    public void testAnnotatedText() {
        try (Transaction tx = getDatabase().beginTx()) {
            ...
            Result news = getDatabase().execute("MERGE (n:News {text: {value}}) WITH n\n"
                    + "CALL ga.nlp.annotate({text:n.text, id: {id}, textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor'}) YIELD result\n"
    ...
    public void testAnnotatedTextWithSentiment() {
        try (Transaction tx = getDatabase().beginTx()) {
            ...
            params.put("id", id);
            Result news = getDatabase().execute("MERGE (n:News {text: {value}}) WITH n\n"
                    + "CALL ga.nlp.annotate({text:n.text, id: {id}, textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor'}, pipeline: \"tokenizerAndSentiment\"}) YIELD result\n"
                    + "MERGE (n)-[:HAS_ANNOTATED_TEXT]->(result)\n"
                    + "return result", params);
            ResourceIterator<Object> rowIterator = news.columnAs("result");
            assertTrue(rowIterator.hasNext());
    ...
    public void testAnnotatedTextAndSentiment() {
        try (Transaction tx = getDatabase().beginTx()) {
            ...
            params.put("id", id);
            Result news = getDatabase().execute("MERGE (n:News {text: {value}}) WITH n\n"
                    + "CALL ga.nlp.annotate({text:n.text, id: {id}, store: true, textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor'}}) YIELD result\n"
                    + "MERGE (n)-[:HAS_ANNOTATED_TEXT]->(result)\n"
                    + "return result", params);
            ResourceIterator<Object> rowIterator = news.columnAs("result");
            assertTrue(rowIterator.hasNext());
    ...
    public void testAnnotatedTextOnMultiple() {
        try (Transaction tx = getDatabase().beginTx()) {
            ...
            Result sentences = getDatabase().execute("MATCH (a:Tweet) WITH a\n"
                    ...
                    + "UNWIND aa AS a\n"
                    + "CALL ga.nlp.annotate({text:a.text, id: id(a), textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor'}}) YIELD result WITH result as at "
...
//            assertEquals("com.graphaware.nlp.processor.stanford.StanfordTextProcessor", resultNode);
        ...
        try (Transaction tx = getDatabase().beginTx()) {
            Result res = getDatabase().execute("CALL ga.nlp.addPipeline({"
                    + "textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor', "
                    + "name: 'testPipe', "
        ...
        try (Transaction tx = getDatabase().beginTx()) {
            Result res = getDatabase().execute("CALL ga.nlp.getPipelines({textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor'}) YIELD result\n"
                    + "return result");
            ResourceIterator<Object> rowIterator = res.columnAs("result");
        ...
        try (Transaction tx = getDatabase().beginTx()) {
            ...
            Result res = getDatabase().execute("MERGE (n:News {text: {value}}) WITH n\n"
                    + "CALL ga.nlp.annotate({text:n.text, id: {id}, textProcessor: 'com.graphaware.nlp.processor.opennlp.OpenNLPTextProcessor', pipeline: 'testPipe'}) YIELD result\n"
                    + "MERGE (n)-[:HAS_ANNOTATED_TEXT]->(result)\n"
                    + "return result", params);
            ResourceIterator<Object> rowIterator = res.columnAs("result");
            assertTrue(rowIterator.hasNext());
